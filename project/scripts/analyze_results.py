...
def main():
    ...
    if args.format == "csv":
        print("model,dataset,prompt_type,accuracy_contain,precision,recall,hallucination_rate,num_samples")
        for r in rows:
            print(",".join(str(r.get(k, "")) for k in ["model", "dataset", "prompt_type", "accuracy_contain", "precision", "recall", "hallucination_rate", "num_samples"]))
        return
    ...
    for r in rows:
        ...
        print(f"  {r['model']:15} | {r['dataset']:18} | {r['prompt_type']:10} | acc={acc_s} | P={p_s} R={q_s} | hall_rate={hall_s} | n={n}")
    print("=" * 80)
